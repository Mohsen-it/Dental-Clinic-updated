...
export default function EditPatientDialog({ isOpen, patient, onClose, onSave }: EditPatientDialogProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    age: 0,
    patient_number: undefined as number | undefined,
    patient_condition: '',
    allergies: '',
    medical_conditions: '',
    email: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check for duplicate patient number (only if patient_number is provided)
    if (formData.patient_number) {
      const existingPatient = patients.find(p =>
        p.patient_number === formData.patient_number && p.id !== patient.id
      )

      if (existingPatient) {
        toast({
          title: "خطأ في رقم المريض",
          description: `يوجد مريض آخر برقم ${formData.patient_number} (${existingPatient.full_name}). يرجى اختيار رقم مختلف.`,
          variant: "destructive",
        })
        return
      }
