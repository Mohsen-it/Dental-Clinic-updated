...
export default function EditInventoryDialog({
  ...
  const handleSubmit = async (data: InventoryFormData) => {
    ...
    setIsLoading(true)
    try {
      // Use custom category/supplier if provided
      const finalData = {
        ...data,
      ...
      await onSave(item.id, finalData)
      onClose()
      toast({
        title: 'نجح',
      ...
      toast({
        title: 'خطأ',
        description: 'فشل في تحديث العنصر. يرجى المحاولة مرة أخرى',
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
